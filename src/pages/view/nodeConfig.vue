		...
		<v-card class="px-6">
			...
			<v-data-table align="center" :headers="headers" disable-sort :items="items">
				...
				<template v-slot:item.oper="{item}">
					<v-btn fab x-small depressed title="删除" class="mx-1" @click="nodeDelete(item.id)" :style="[theme.bg_a,theme.co_p]"
					 v-if="item.canD!==1">
						<v-icon>iconfont iconfont-customerarchivesrecycleBin</v-icon>
					</v-btn>
					<v-btn fab x-small depressed title="修改" class="mx-1" @click="editnode(item.id)" v-if="item.canD!==1" :style="[theme.bg_a,theme.co_p]">
		...
		<v-dialog v-model="dialog" persistent class="v-dialog">
			<v-row justify="center" v-if="dialog">
				<v-col cols="8" class="pa-0 ma-0">
					<v-card class="pa-5">
						<v-card-title class="justify-center text-uppercase text-h5 py-2">{{dialogType=='add'?'添加':'更新'}}节点</v-card-title>
						<v-card-text>
							<v-row>
								<v-col cols="6">
								...
								<v-col cols="6">
									<v-text-field label="import引入路径" persistent-hint hint="不要以/开头" v-model="nodeModel.component"></v-text-field>
								</v-col>
								<v-col cols="6">
									<v-select label="父节点" v-model="nodeModel.cid" :items="parentNode" item-text="call" item-value="self"></v-select>
								</v-col>
								<v-col cols="4">
								...
								<v-col cols="4" class="d-flex flex-row align-center">
									...
									<v-radio-group row class="ml-10" v-model="nodeModel.auth">
										<v-radio label="用户" value="user" off-icon="iconfont-weixuan" on-icon="iconfont-xuanzhong"></v-radio>
										<v-radio label="管理员" value="admin" off-icon="iconfont-weixuan" on-icon="iconfont-xuanzhong"></v-radio>
									</v-radio-group>
								</v-col>
								...
								<v-col cols="12" class="pt-0">
									...
									<v-sheet>
										<v-btn icon v-for="(icon,idx) in icons" :key="idx" class="mx-1" :color="nodeModel.icon===icon?theme.bg_p.background:''"
						...
						<v-card-actions class="justify-center">
							<v-btn width="120" class="mx-2" @click="submit(dialogType)" :style="[theme.bg_p,theme.co]">{{dialogType=='add'?'提交':'更新节点'}}</v-btn>
							<v-btn width="120" class="mx-2" @click="nodeModelReset(1)" :style="[theme.bg_p,theme.co]">关闭</v-btn>
						</v-card-actions>
					</v-card>
				</v-col>
			</v-row>
	...
	export default {
		...
		methods: {
			...
			async submit(type) {
				...
				if (_node.deep > 3) return that.$hint({
					msg: "节点过深",
					type: "error"
				});
